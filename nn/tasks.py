...
def parse_model(d, ch, verbose=True):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in {
            ...
            c2 = ch[f[-1]]
        elif m is FPN:  # Handle FPN block
            in_channels = [ch[x] for x in f]
            out_channels = args[0]
            args = [in_channels, out_channels]
            c2 = out_channels
        elif m is AdaptivePyramidScalingFPN:
            in_channels = [ch[x] for x in f]

            # Hợp nhất số kênh nếu `f` là danh sách nhiều đầu vào
            if len(in_channels) > 1:
                in_channels = sum(in_channels)  # Tính tổng số kênh từ các đầu vào
            else:
                in_channels = in_channels[0]  # Lấy giá trị duy nhất nếu chỉ có một đầu vào

            out_channels = args[0]
            # args = [in_channels, out_channels]
            pyramid_levels = args[1] if len(args) > 1 else 5
            m_ = AdaptivePyramidScalingFPN(
                in_channels=in_channels,
                out_channels=out_channels,
                pyramid_levels=pyramid_levels
            )
            # Cập nhật `c2` là số kênh đầu ra của APS-FPN
            c2 = out_channels
        else:
            c2 = ch[f]
