...
class FPN(nn.Module):
    """
    Feature Pyramid Network (FPN) for combining multi-scale features from the backbone.
    """

    def __init__(self, in_channels_list, out_channels):
        """
        Args:
            in_channels_list (list[int]): Number of channels for each input feature map.
            out_channels (int): Number of output channels for the final FPN output.
        """
        ...
        # Final convolution to reduce concatenated channels to `out_channels`
        self.final_conv = nn.Conv2d(len(in_channels_list) * out_channels, out_channels, kernel_size=1)

    def forward(self, inputs):
        """
        Args:
            inputs (list[Tensor]): Feature maps from the backbone.
                List order: [low-resolution, ..., high-resolution].
        Returns:
            Tensor: Unified feature map with `out_channels` channels.
        """
        assert len(inputs) == len(self.lateral_convs), \
            f"Expected {len(self.lateral_convs)} inputs, got {len(inputs)}"

        # Apply lateral convolutions
        laterals = [lateral_conv(feat) for lateral_conv, feat in zip(self.lateral_convs, inputs)]

        # Build top-down path
        for i in range(len(laterals) - 1, 0, -1):  # Start from the highest resolution
        ...
        # Global pooling and concatenate features
        concat_features = torch.cat([F.adaptive_avg_pool2d(o, 1) for o in outputs], dim=1)
        # Reduce channels to `out_channels`
        outputs = self.final_conv(concat_features)
        print("output shape of FPN:", outputs.shape)
        return outputs
    
# End Feature Pyramid Network

# Adaptive Pyramid Scaling FPN
class AdaptivePyramidScalingFPN(nn.Module):
    def __init__(self, in_channels, out_channels, pyramid_levels=5):
        ...
        """
        super(AdaptivePyramidScalingFPN, self).__init__()
...
# End Adaptive Pyramid Scaling FPN
